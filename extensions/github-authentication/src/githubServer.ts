...
const REDIRECT_URL_STABLE = 'https://billai.dev/redirect';
const REDIRECT_URL_INSIDERS = 'https://billai.dev/redirect';
...
export class GitHubServer implements IGitHubServer {
	...
	private async getRedirectEndpoint(): Promise<string> {
		...
		if (this._type === AuthProviderType.github) {
			...
			// If we are running in insiders billai.dev, then ensure we use the redirect route on that.
			this._redirectEndpoint = REDIRECT_URL_STABLE;
			if (proxyEndpoints?.github && new URL(proxyEndpoints.github).hostname === 'insiders.billai.dev') {
				this._redirectEndpoint = REDIRECT_URL_INSIDERS;
			}
		} else {
			// GHE only supports a single redirect endpoint, so we can't use
			// insiders.billai.dev/redirect when we're running in Insiders, unfortunately.
			...
			this._redirectEndpoint = 'https://billai.dev/redirect';
		}
		return this._redirectEndpoint;
	}
