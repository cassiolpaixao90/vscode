...
// Make sure to sync BillAI state here
...
onceDocumentLoaded(() => {
	...
	if (typeof scrollProgress === 'number' && !settings.settings.fragment) {
		doAfterImagesLoaded(() => {
			...
			// Always set scroll of at least 1 to prevent BillAI's webview code from auto scrolling us
...
document.addEventListener('click', event => {
	...
	while (node) {
		if (node.tagName && node.tagName === 'A' && node.href) {
			...
			// If original link doesn't look like a url, delegate back to BillAI to resolve
