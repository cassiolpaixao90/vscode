...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in BillAI and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in BillAI
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in BillAI
			case 'csv':
			case 'toml':
