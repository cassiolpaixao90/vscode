# BillAI Copilot Instructions

## Project Overview

BillAI is built with a layered architecture using TypeScript, web APIs and Electron, combining web technologies with native app capabilities. The codebase is organized into key architectural layers:

### Root Folders
- `src/`: Main TypeScript source code with unit tests in `src/vs/*/test/` folders
- `build/`: Build scripts and CI/CD tools
- `extensions/`: Built-in extensions that ship with BillAI
...
- `src/vs/workbench/` - Main application workbench for web and desktop
  ...
  - `workbench/api/` - Extension host and BillAI API implementation
...
The `extensions/` directory contains first-party extensions that ship with BillAI:
...
Each extension follows the standard BillAI extension structure with `package.json`, TypeScript sources, and contribution points to extend the workbench through the Extension API.
...
MANDATORY: Always check the `BillAI - Build` watch task output via #runTasks/getTaskOutput for compilation errors before running ANY script or declaring work complete, then fix all compilation errors before moving forward.

- NEVER run tests if there are compilation errors
- NEVER use `npm run compile` to compile TypeScript files but call #runTasks/getTaskOutput instead

### TypeScript compilation steps
- Monitor the `BillAI - Build` task outputs for real-time compilation errors as you make changes
- This task runs `Core - Build` and `Ext - Build` to incrementally compile BillAI TypeScript sources and built-in extensions
