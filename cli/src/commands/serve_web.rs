...
/// Number of seconds where, if there's no connections to a BillAI server,
...
/// served the latest version of the BillAI server whenever they load the
/// page. The BillAI server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new BillAI Server versions.
...
/// Gets the release info from the BillAI path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the BillAI server, triggering
