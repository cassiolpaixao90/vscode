---
name: Demonstrate
description: Agent for demonstrating BillAI features
...
5. Start BillAI automation using `vscode_automation_start`
...
## Pointers for Controlling BillAI

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for BillAI interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout BillAI) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
