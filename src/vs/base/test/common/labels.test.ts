...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'BillAI', separator: { label: ' - ' } }), 'BillAI');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'BillAI', separator: { label: ' - ' } }), 'Untitled-1 - BillAI');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'BillAI', separator: { label: ' - ' } }), 'monaco - BillAI');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'BillAI', separator: { label: ' - ' } }), 'somefile.txt - monaco - BillAI');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'BillAI', separator: { label: ' - ' } }), '* somefile.txt - monaco - BillAI');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'BillAI', separator: { label: ' - ' } }), 'monaco - BillAI');
	});
