...
export abstract class AbstractTaskService extends Disposable implements ITaskService {
	...
	private _openDocumentation(): void {
		this._openerService.open(URI.parse('https://billai.dev/docs/editor/tasks#_defining-a-problem-matcher'));
	}
				...
				if (active) {
					this._terminateAll().then((responses) => {
						...
						if (response.code && response.code === TerminateResponseCode.ProcessNotFound) {
							this._notificationService.error(nls.localize('TerminateAction.noProcess', 'The launched process doesn\'t exist anymore. If the task spawned background tasks exiting BillAI might result in orphaned processes.'));
						} else {
