...
if (-not $env:VSCODE_PYTHON_AUTOACTIVATE_GUARD) {
	...
	if ($env:VSCODE_PYTHON_PWSH_ACTIVATE -and $env:TERM_PROGRAM -eq 'vscode') {
		...
		try {
			...
			$activationError = $_
			Write-Host "`e[0m`e[7m * `e[0;103m BillAI Python powershell activation failed with exit code $($activationError.Exception.Message) `e[0m"
		}
	}
}
...
# Set always on key handlers which map to default BillAI keybindings
