...
/*
 * Implements a standalone CLI app that opens BillAI from a remote terminal.
 ...
 *  - In external terminals for WSL this calls BillAI on the Windows side.
 *    The BillAI desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a BillAI terminal.');
		return;
	}
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to BillAI server: ' + message);
	console.error(err);
	process.exit(1);
}
