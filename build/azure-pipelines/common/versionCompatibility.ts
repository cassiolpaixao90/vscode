...
export function isEngineCompatible(productVersion: string, engineVersion: string): { compatible: boolean; error?: string } {
	...
	if (prefix === '>=') {
		...
		if (pMajor < reqMajor) { return { compatible: false, error: `Extension requires BillAI >=${engineVersion}, but product version is ${productVersion}` }; }
		if (pMinor > reqMinor) { return { compatible: true }; }
		if (pMinor < reqMinor) { return { compatible: false, error: `Extension requires BillAI >=${engineVersion}, but product version is ${productVersion}` }; }
		if (pPatch >= reqPatch) { return { compatible: true }; }
		return { compatible: false, error: `Extension requires BillAI >=${engineVersion}, but product version is ${productVersion}` };
	}

	// Caret or exact version check
	if (pMajor !== reqMajor) {
		return { compatible: false, error: `Extension requires BillAI ${engineVersion}, but product version is ${productVersion} (major version mismatch)` };
	}
...
export function areApiProposalsCompatible(
	...
	for (const { proposalName, version } of parsedProposals) {
		...
		if (!existingProposal) {
			errors.push(`API proposal '${proposalName}' does not exist in this version of BillAI`);
		} else if (existingProposal.version !== version) {
			errors.push(`API proposal '${proposalName}' version mismatch: extension requires version ${version}, but BillAI has version ${existingProposal.version ?? 'unversioned'}`);
		}
	}
