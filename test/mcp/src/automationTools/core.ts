...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	tools.push(server.tool(
		...
		'Restart BillAI with optional workspace or folder and extra command-line arguments',
		{
		...
		async ({ workspaceOrFolder, extraArgs }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: `BillAI restarted successfully${workspaceText}${argsText}`
				}]
			};
		}
	));
	...
	tools.push(server.tool(
		...
		async () => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'BillAI stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of BillAI (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 	'Send a keybinding to BillAI (e.g., ctrl+shift+p, cmd+s)',
	// 	{
