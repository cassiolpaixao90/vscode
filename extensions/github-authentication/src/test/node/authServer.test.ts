...
suite('LoopbackAuthServer', () => {
	...
	setup(async () => {
		server = new LoopbackAuthServer(__dirname, 'http://localhost:8080', 'https://billai.dev');
	...
	test('should resolve with code and state on /callback with valid parameters', async () => {
		...
		assert.strictEqual(response.status, 302);
		assert.strictEqual(response.headers.get('location'), `/?redirect_uri=https%3A%2F%2Fbillai.dev&app_name=${encodeURIComponent(env.appName)}`);
		await Promise.race([
