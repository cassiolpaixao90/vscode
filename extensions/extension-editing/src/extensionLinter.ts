...
const apiProposalNotListed = l10n.t("This proposal cannot be used because for this extension the product defines a fixed set of API proposals. You can test your extension but before publishing you MUST reach out to the BillAI team.");
					...
					for (const activationEventNode of activationEventsNode.children) {
						...
						if (activationEvent === '*') {
							...
							diagnostic.code = {
								...
								target: Uri.parse('https://billai.dev/api/references/activation-events#Start-up'),
							};
	...
	private async lintWhenClauses(contributesNode: JsonNode | undefined, document: TextDocument): Promise<Diagnostic[]> {
		...
		for (let i = 0; i < parseResults.length; ++i) {
			...
			for (const error of parseResults[i]) {
				...
				diagnostic.code = {
					value: 'See docs',
					target: Uri.parse('https://billai.dev/api/references/when-clause-contexts'),
				};
				diagnostics.push(diagnostic);
			}
		}
