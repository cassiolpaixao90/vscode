...
export async function getServer(appService: ApplicationService): Promise<Server> {
	const server = new McpServer({
		name: 'BillAI Automation Server',
		version: '1.0.0',
		title: 'An MCP Server that can interact with a local build of BillAI. Used for verifying UI behavior.'
	}, { capabilities: { logging: {} } });

	server.tool(
		'vscode_automation_start',
		'Start BillAI Build. If workspacePath is not provided, BillAI will open with the last used workspace or an empty window.',
		{
		...
		async ({ recordVideo, workspacePath }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: app ? `BillAI started successfully${workspacePath ? ` with workspace: ${workspacePath}` : ''}` : `Failed to start BillAI`
				}]
			};
		}
	);

	// Apply all BillAI automation tools using the modular structure
