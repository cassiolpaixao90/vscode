...
- Open `/extensions/json-language-features/` as the workspace in BillAI
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/json-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new BillAI instance with the `json-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  BillAI window opened on `json-language-features`.
...
- Open both `vscode-json-languageservice` and this extension in two windows or with a single window with the[multi-root workspace](https://billai.dev/docs/editor/multi-root-workspaces) feature.
